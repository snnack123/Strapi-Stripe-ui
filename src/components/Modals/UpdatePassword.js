...
import { updatePasswordSchema } from '../../utils/yupSchemas';
import { updatePasswordData } from '../../utils/constants';
import { updateUser } from '../../utils/userRequests';

export default function UpdatePassword() {
    const { jwt } = useSelector((state) => state.user_store);
    const { updatePassword } = useSelector((state) => state.modal_store);
    ...
    const handleModal = (type) => {
        dispatch({ type: 'modal/updatePassword', payload: type });
        setUpdatedPasswordError({ error: false, message: '' });
    }

    const updatePasswordHandler = async (password) => {
        const result = await updateUser('password', { password }, jwt);

        if (!result.status) {
            setUpdatedPasswordError({ error: true, message: result.message });
        } else {
            handleModal(false);
            dispatch({ type: 'modal/showUpdatedPassword', payload: true });
        }
    }
                                                    ...
                                                    <Form className='space-y-8 divide-y divide-gray-200'>
                                                        <div className="space-y-6 pt-8 sm:space-y-5 sm:pt-10">
                                                            <div className="space-y-6 sm:space-y-5">
                                                                ...
                                                                <div className="sm:col-span-3">
                                                                    <label htmlFor="passwordConfirmation" className="block text-sm font-medium leading-6 text-gray-600">
                                                                        Confirm Password*
                                                                    </label>
                                                                    <div className="mt-2">
                                                                        <Field
                                                        ...
                                                        <div className='border-none text-red-500 text-xs italic text-center'>{updatedPasswordError.message}</div>
